...
public class WebFluxConfiguration {
    ...
    public RouterFunction<ServerResponse> routerFunctioUsers(UserRepository userRepository) {
        // 获取所有的userList
        Collection<UserBean> userList = userRepository.findAll();
        // 创建Flux 包含所有的userList
        Flux<UserBean> userListFlux = Flux.fromIterable(userList);
        ...
        return RouterFunctions.route(RequestPredicates.path("/users"),
                ...
                flux 0 - N 个 元数
                 */
                request -> ServerResponse.ok().body(userListFlux, UserBean.class));
    }
}